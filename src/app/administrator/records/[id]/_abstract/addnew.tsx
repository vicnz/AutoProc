...
const AddNewDocument = function (props: { data: any, id: string }) {
    ...
    return (
        <Result
            status='404'
            title="No Document"
            subTitle={`Not yet created an Abstract of Quotation Document`}
            extra={(
                <Button
                    loading={loading}
                    ...
                    onClick={async () => {
                        setLoading(true)
                        let result = await fetch(`/administrator/api/records/abstract?_id=${props.id}`, { method: 'POST' })
                        if (result.ok) {
                            message.success(`Added New BAC Resolution for ${props.data?.pr?.pr_no || 'not defined'}`)
                            setLoading(false)
                            mutate(`/administrator/api/records/abstract?_id=${props.id}`)
                        } else {
                            setLoading(false)
