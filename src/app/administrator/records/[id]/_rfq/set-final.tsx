...
const MarkedFinal = function (props: { id: string }) {
    ...
    const setFinal = async () => {
        setLoading(true)
        setOpen(true)
        const data = await fetch(`/administrator/api/records/rfq?_id=${props.id}&_reqtype=${true}`, { method: 'PUT' })

        if (data.ok) {
            message.info('Request For Price Quotation, Marked as Completed!')
            setOpen(false)
            setLoading(false)
        } else {
