...
const PurchaseOrderFormEdit = function (props: AddNewPurchaseRequestProps) {
    ...
    return (
        <>
            {props.type === "add" ? (
                <>
                    <Button icon={<PlusCircleOutlined />} type="text" onClick={() => setOpen(true)}>
                        {props.children || "Add PO"}
                    </Button>
                    <Drawer destroyOnClose open={open} onClose={onClose} closable={true} title="Add PO">
                        <PurchaseOrderForm close={() => setOpen(false)} isEdit={false} prID={props.prId} />
                    </Drawer>
                </>
            ) : (
                <>
                    <Button icon={<EditOutlined />} type="primary" onClick={() => setOpen(true)} disabled={props.final}>
                        {props.children || "Edit PO"}
                    </Button>
                    <Drawer destroyOnClose open={open} onClose={onClose} closable={true} title="Edit PO">
