    ...
    return (
        <div style={{ padding: "5px 25px" }}>
            ...
            <div>
                ...
                {/* Letter Opening */}
                <br />
                <Paragraph style={{ textIndent: "3em", lineHeight: "1.8em" }}>
                    We are happy to notify you that your quotation dated {date && dayjs(date).format("MMMM DD, YYYY")}{" "}
                    for the <Text italic>{`"${purpose}"`}</Text>, with PR{" "}
                    <Text style={{ color: token.colorPrimary }}>#{number}</Text> for the contract price equivalent to{" "}
                    {amount && (NumberToWords(amount) as string).toUpperCase()} PESOS ONLY (
                    {amount &&
                ...
                <Paragraph style={{ textIndent: "3em", lineHeight: "2em" }}>
                    However, If you already have a maintaining and updated file of the above-mentioned requirements in
                    the BAC Office you may no longer require its re-submission (Annex &lsquo;H&rsquo;, Appendix
